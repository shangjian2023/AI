...
按 ADR-0037，上述 2 后门 + 5 clean 可以复用为 `development_reuse` 竞赛指标，允许展示
混淆矩阵和开发集性能，但必须明确 `calibration_overlap=true`。独立 blind 样本不再是继续
展示或扩展模型族的前置条件。该放宽不改变 ADR-0024 的 `formal_blind` 聚合边界，也不把
开发样本重新定义为 blind；正式泛化声明仍需要未参与参数选择的新模型。

ADR-0038 将后续阈值更新改为版本化的全局优先拟合。队长侧工具从已验收开发报告的同候选
对数似然差与族支持网格中，在 clean FPR 约束下选择最高召回规则，并输出留一 fold 与
bootstrap 稳定性。只有模型族至少包含 3 clean + 3 backdoor，且分族阈值使 F1 提升至少
0.10、不增加允许的 clean FPR，才生成专属 override。组员采集配置的族支持 3 只是扩大
Top-6 原始证据覆盖，不是新的展示门槛；活跃平台仍使用 v2，直到整批新报告验收并显式提升。

多模型外部分工由 ADR-0039 的 source-only ZIP 承载。参数化 runner 固定 matched pair 配置、
断点恢复和真值隔离；成功回传强制包含 backdoor/clean 两份 Adapter、全部报告、软触发工件、
日志和环境清单。ZIP 在组员端二次校验文件 SHA 与 Adapter/probe 指纹后才打印
`RETURN_READY`；失败则保留进度并生成 failure ZIP，执行 AI 无权修改阈值或实验预算。

ADR-0040 为跨模型潜变量探测增加有限值契约。模型仍按检测配置以 FP16 存储；支持 BF16
的 CUDA 设备在 soft prompt 优化、refinement 和回放期间使用 BF16 autocast，并在报告
`runtime` 中记录实际计算精度。任一 logits、loss、概率、梯度或保存向量出现非有限值时
立即失败，不生成可恢复的完整 probe；组员 runner 也严格拒绝包含非标准 `NaN/Infinity`
常量的本地报告和回传 ZIP。该修复不改变候选、阈值、seed、batch 或已完成 mining 摘要。
...
ADR-0041 在新 mining 报告中增加去重前候选 audit，并按共享精确 suffix 的不同 seed token
计算 `pre_deduplication_family_support`。该值和 `candidate_family_audit` 均为
`decision_use=false`；现役 `family_support` 仍只基于去重后的完整候选集，候选选择及平台
`2.0 + 支持 5` 语义不变。旧报告缺少 audit 时继续兼容读取且不猜测回填。相同 ADR 还提供
与完整 probe 共用固定初始化的全输入 step-0 只读评分 API；它不反向传播或更新模型，目前
未接入 CLI、候选预算或判决，也不能直接使用历史首 batch step-0 的开发阈值。

ADR-0042 将论文对齐差异改为显式协议，而不追溯改变历史报告。`mine` 与 `merge` 默认保持
`single_best`；新运行可从每个分片开始选择 `seed_preserving` 或实验性的
`dual_metric_cluster`，报告同时记录分片与合并策略。训练侧 `training.response_prefix`、
检测侧 `mining.response_prefix` 分别贯通训练/质量门和 mining/probe/replay，probe 输入清单
保存实际边界 token IDs；response-only mask 与完整输入使用相同 special-token 编码，避免
OPT 自动 BOS 引起的标签错位。概率判据由 `probe.probability_gap_mode` 区分历史有方向差
`directional` 与论文绝对差 `paper_absolute`，候选报告同时保存 signed、absolute 和实际判定
统计量。该模式不改写平均 token 对数似然、候选族支持或平台展示 profile。
...
| `scripts/fit_competition_development_thresholds.py` | 跨模型开发报告的全局阈值、稳定性与受限分族 override 拟合 |
| `scripts/build_team_model_bundles.py` | 构建论文模型矩阵的确定性 source-only 组员 ZIP |
| `scripts/run_team_model_pair.py` | 组员 matched pair 运行、恢复、强制回传与 ZIP 自校验 |
