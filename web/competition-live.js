  ...
  function create(deps) {
    ...
    function renderCompetitionProbe() {
      ...
      $("rfProbeProgress").textContent = probe.evidence
        ? `候选 #${probe.rank} 完成 · 对数似然差 ${Number(probe.max_log_likelihood_gap || 0).toFixed(4)}`
        : `候选 #${probe.rank} 正在探测`;
      $("rfProbeInputs").innerHTML = `<div><span>候选族支持度</span><code>${escapeHtml(probe.family_support ?? "等待结果")}</code></div><div><span>论文复现记录</span><code>${probe.criterion_met == null ? "计算中" : probe.criterion_met ? "概率差已越过 0.25（不裁决）" : "概率差未越过 0.25（不裁决）"}</code></div>`;
